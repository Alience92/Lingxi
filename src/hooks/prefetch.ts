...
async function main() {
  ...
  console.log(`[AgentMemory] debug: query="${userMessage.slice(0, 80)}", projects=${projectIds.join(",")}`);

  for (const pid of projectIds) {
    try {
      const pre = await prefetch([{ role: "user", text: userMessage }], pid);
      console.log(`[AgentMemory] debug: project=${pid} confidence=${pre.confidence.toFixed(3)} fragmentIds=${pre.fragmentIds.join(",")}`);
